...
class lightDPR5Dataset(BaseDataset):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        for i in range(0, len(self.AB_paths_) - 5, 5):
            path_list = self.AB_paths_[i:i + 5]
            sampled_list = random.sample(path_list, 5)
            for jk in sampled_list:
                # remaining_list = list(set(self.AB_paths_[i:i + 5]) - set(list(jk)))
                remaining_list = self.AB_paths_[i:i + 5]
                remaining_list.remove(jk)
