...
class lightDPRsingle1024Dataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        for i in range(0, len(self.AB_paths_) - 5, 5):
            sampled_list = random.sample(self.AB_paths_[i:i + 5], 1)
    ...
    def __getitem__(self, index):
        ...
        im_list_nums = ['00','01','02','03','04']
        random.shuffle(im_list_nums)
        sampled_s_t = random.sample(im_list_nums, 2)
        random.shuffle(sampled_s_t)


        AB_path = self.AB_paths[index][:-6]+sampled_s_t[0]+'.png'
        A = cv2.imread(AB_path)
        img_A = cv2.resize(A, (1024, 1024))
        ...
        if self.opt.isTrain:
            target_path = AB_path[:-6]+sampled_s_t[1]+'.png'
        ...
        img_orig = cv2.resize(orig, (1024, 1024))
    ...
    def name(self):
        return 'lightDPRsingle1024Dataset'
