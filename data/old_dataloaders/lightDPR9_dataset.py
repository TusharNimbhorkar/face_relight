...
class lightDPR9Dataset(BaseDataset):
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        for i in range(0, len(self.AB_paths_) - 6, 6):
            ...
            i1 = self.AB_paths_[i:i + 5]
            overall = []
            for r in itertools.product(i1, i1):
                overall.append([r[0],r[1]])
            random.shuffle(overall)
            for kk in random.sample(overall,10):
                self.AB_paths.append(kk[0])
                self.list_AB.append([kk[0], kk[1]])
