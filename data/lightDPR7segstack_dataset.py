...
class lightDPR7segstackDataset(BaseDataset):
    @staticmethod
    ...
    def __getitem__(self, index):
        ...
        return {'A': A, 'B': B, 'C': C, 'D':torch.cat((D,S)), 'A_con':torch.cat((A,S)), 'AL':torch.from_numpy(sh_AL),'BL':torch.from_numpy(sh_BL), 'A_paths': AB_path, 'B_paths': AB_path}

    def __len__(self):
        return len(self.list_AB)

    def name(self):
        return 'lightDPR7segstackDataset'
