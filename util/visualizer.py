        ...
        if self.display_id > 0:  # show images in the browser
            ...
            if ncols > 0:
                ...
                for label, image in visuals.items():
                    ...
                    if image_numpy.shape[2] == 12:
                        # Todo: maybe not image?? just leave it as np array
                        im1_np = image_numpy[:, :, :3]
                        im2_np = image_numpy[:, :, 3:]
                        image_pil = Image.fromarray(im1_np)
                        # im2 = Image.fromarray(im2_np)
                        # image_pil = Image.blend(im1, im2, alpha=0.0)
                    else:
                        image_pil = Image.fromarray(image_numpy)

                    image_numpy = np.copy(image_pil)
