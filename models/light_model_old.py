...
class lightModel(BaseModel):
    ...
    def forward(self,epoch):
        ...
        # print(count_skip)
        # print(count_skip)
        self.fake_B, self.fake_AL, self.face_feat_A = self.netG(self.real_A,self.real_BL,count_skip)
        if epoch>10:
    ...
    def backward_G(self,epoch):
        ...
        self.loss_G_total_variance =  torch.mean(torch.abs((torch.mean(
            torch.abs(self.real_B[:, :, :, :-1] - self.real_B[:, :, :, 1:])) + torch.mean(
            torch.abs(self.real_B[:, :, :-1, :] - self.real_B[:, :, 1:, :]))) - (torch.mean(
            torch.abs(self.fake_B[:, :, :, :-1] - self.fake_B[:, :, :, 1:])) + torch.mean(
            torch.abs(self.fake_B[:, :, :-1, :] - self.fake_B[:, :, 1:, :])) )))
