...
class lightgrad61Model(BaseModel):
    def name(self):
        return 'lightgrad61Model'
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'G_L1', 'G_L1_SS','G_MSE', 'G_total_variance', 'G_feat', 'L1_add', 'G', 'D_real', 'D_fake', 'D']
    ...
    def set_input(self, input):
        ...
        self.real_A = input['A'].to(self.device)#source
        self.real_B = input['B'].to(self.device)#target
        self.real_AL = input['AL'].to(self.device)#source light
        self.real_BL = input['BL'].to(self.device)#target light
        self.real_C = input['C'].to(self.device)#realffhq
        self.real_D = input['D'].to(self.device)#real_celeba_of_source_target
    ...
    def forward(self, epoch):
        ...
        if epoch <= 10:
            ...
            self.fake_BA, _, _, _ = self.netG(self.real_A, self.fake_AL, count_skip, oriImg=None)
    ...
    def backward_G(self, epoch):
        ...
        self.loss_G_L1_SS = self.criterionL1(self.real_A, self.fake_BA)  # * self.opt.lambda_L1
