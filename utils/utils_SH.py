...
def convert_sh_to_3dul(sh):
        right_most = shtools_sh2matrix(sh,2)
        shMatrix = SHRotateRealCoef(right_most, np.array([3*np.pi/2, 0*np.pi, 0*np.pi]), djpi2(2))

        sh_rot = shtools_matrix2vec(shMatrix)
        return sh_rot
